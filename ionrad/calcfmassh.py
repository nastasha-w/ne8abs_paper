...
Note: functions currently use EAGLE EOS stuff. Should NOT use this for 
FIRE without modifying first.
...
def linterptab(z1, z2, tab1, tab2, zint):
    ...
    '''
    return {key: linterp(z1, z2, tab1[key], tab2[key], zint) 
            for key in tab1.keys()}
...
def ABCquadeq(dct_ABC):
    ...
    totaylor = 3e-4 
    # x for minimum relative difference of taylor and nominal 
    # calculations of 1 - sqrt(1 -x), at difference of ~6e-5 
    # done for float32 since that is what the EAGLE arrays used here
    # are stored in
    out = np.empty(A.shape, dtype=np.float32)
    tempsel = np.abs(C) >= totaylor # 1 array 
    out[tempsel] = B[tempsel] / (2. * A[tempsel]) \
                   * (1. - np.sqrt(1. - C[tempsel])) # 1 array, 1 temp
    np.logical_not(tempsel, out=tempsel)
    # taylor: 1 - sqrt(1-x) ~ 1 - (1 - 1/2 x) = 1/2 x
    out[tempsel] = B[tempsel] * C[tempsel] / (4. * A[tempsel]) #1 arr, 1 temp
    del tempsel # -1 array
    del A
    del B
    del C
...
def getfitparams(z): 
    ...
    not optimised for speed; designed for use for a single redshift 
    and many particles
    ...
    if z < 1.:
        ...
        if z != maxz:
            msg = (f'Warning: redshift {z:.2f} outside interpolation range;'
                   f'using redshift {maxz:.2f} values')
            print(msg)
...
def getUVBparams(z, UVB='HM01'):
    ...
    zs = np.array(list(phototable.keys())) 
    maxz = np.max(zs)
    if z >= maxz:
        if z != maxz:
            msg = (f'Warning: redshift {z:.2f} outside interpolation range;'
...
def Gammaphot_over_GammaUVB(dct_nH, z):
    ...
    Gammaphot/GammaUVB = (1. -f)*(1 + (nH/n0)**beta)**alpha1 
                         + f*(1 + nH/n0)**alpha2
    '''
    ...
    return onemf*(1. + (dct_nH['nH'] / n0)**beta)**alpha1 \
           + (1. - onemf) * (1 + dct_nH['nH'] / n0)**alpha2
...
def LambdaT(dct_T):
    '''
    Rahmati, Pawlik, Raicevic, Schaye 2013, equation A6, 
    citing Theuns et al. 1998
    output units : cm3/s 
    input units: K?
    '''
    return 1.17e-10 * dct_T['Temperature']**0.5 \
           * np.exp(-157809. / dct_T['Temperature']) \
           / (1. + np.sqrt(dct_T['Temperature'] * 1.0e-5))
...
def alphaA(dct_T):
    ...
    return 1.269e-13 * lambdapar**1.503 \
           / (1. + (lambdapar / 0.522)**0.47)**1.923 
    
def nHIHmol_over_nH(dct_nH_T, z, UVB='HM01', useLSR=False):
    '''
    HM01: used in EAGLE
    LSR: local stellar radiation (analytical model, not based on stars
         in simulation)
    Rahmati, Pawlik, Schaye, Raicevic equation A8, eta in text above A4
    does not account for molecular hydrogen, so I will assume this also
    reflects the mass density ratios (i.e., that some of the HI 
    combining into H2 does not cause more HI to form; seems reasonable 
    in that this fraction would be close to one before H2 forms in 
    ...
    if useLSR:
        B = (Gammaphot_over_GammaUVB(dct_nH_T,z) * UVB + GammaLSR(dct_nH_T)) \
            / dct_nH_T['nH']   # sets 1 array, 1 temp
    else:
    ...
    del B
    del C
    h1hmolfrac[h1hmolfrac > 1.] = 1. # handle numerical nHI > nH situations 
                                     # 1 temp 
...
def rhoHmol_over_rhoH(dct_nH_T_eos, EOS='eagle'):
    ...
    citing Altay et al. 2011 good obs. fit,
    Blitz and Rosolowsky 2006 scaling
    ...
    # EAGLE EOS: 8000 K at 0.1 cm^-3, 
    # then T = 8000.*( nH / 0.1 cm^-3)**(1./3.) at higher nH
    if EOS == 'owls': # assumes gas is on the EOS
        return 1. / (1. + 24.54 * (dct_nH_T_eos['nH'] / 0.1)**-1.23) # 1 temp
    elif EOS == 'eagle':
        nstar = 0.1 # cm^-3
        Pfloor_nH = 0.1 * 8.0e3 # K cm^-3
        # assuming only H/He, f_H_mass = 0.752, K cm^-3
        Pfloor_ntot = (1 + 0.248 * (0.752**-1 -1.)) * Pfloor_nH 
        gamma_eos = 4. / 3.
        # Blitz and Rosolowsky 2006
        alpha = 0.92
        P0    = 3.5e4 # K cm^-3
        # (Pfloor_nH / P0)**(-1 * alpha) # (Pfloor_nH / P0)**(alpha) # 
        C1 = (Pfloor_ntot / P0)**(-1 * alpha)
        
        # calculate the pressure: 
        # n * T for most gas, EAGLE EOS for star-forming gas
        Pgas_nH = dct_nH_T_eos['Temperature'] * dct_nH_T_eos['nH']
        Pmin_nH = Pfloor_nH * (dct_nH_T_eos['nH'][dct_nH_T_eos['eos']] \
                               / nstar)**gamma_eos
        #Pmin_nH = Pfloor_nH * \
        # (dct_nH_T_eos['nH'][dct_nH_T_eos['nH'] > nstar] \
        #  / nstar)**gamma_eos
        # impose EAGLE pressure floor: important if using 10^4 K for EOS gas
        #Pgas_nH[dct_nH_T_eos['eos']] = np.maximum(Pmin_nH, 
        #    Pgas_nH[dct_nH_T_eos['eos']])
        Pgas_nH[dct_nH_T_eos['eos']] = Pmin_nH
        #Pgas_nH[dct_nH_T_eos['nH'] > nstar] = Pmin_nH
        #del impose_eos
        del Pmin_nH
        # assuming fH is never too far from the primordial value, 
        # the ntot/nH values in the P/Pfloor ratio cancel out
        # otherwise, rho / mu_primordial is used in the simulation 
        # itself so pure n_H isn't the way to go anyway
...
def testtotaylorparameters():
    ...
    title = ('Comparison between 1 - sqrt(1-x) and its'
             ' Taylor approximation at x=1: 0.5x')
    plt.title(title)    
    plt.show()
...
def plotHIfracs(T=1e4,z=3.):
    ...
    plt.subplot(121)
    plt.plot(np.log10(nHs), np.log10(nHIHmol_over_nH(dct_nH_T, z, 
                                                     UVB='HM01', 
                                                     useLSR=False)), 
             label='HM01', color='blue')
    plt.plot(np.log10(nHs), np.log10(nHIHmol_over_nH(dct_nH_T, z, 
                                                     UVB='HM12', 
                                                     useLSR=False)), 
            label='HM12', color='green')    
    plt.plot(np.log10(nHs), np.log10(nHIHmol_over_nH(dct_nH_T, z, 
                                                     UVB='FG09', 
                                                     useLSR=False)), 
            label='FG09', color='red')    
    plt.xlim((-6., 2.))
    plt.ylim(-6.1, 0.2)
    plt.xlabel(r'$\log_{10} n_H \, [\mathrm{cm^{-3}}]$', fontsize=fontsize)
    ylab = r'$\log_{10} (n_{\mathrm{HI}} + n_{\mathrm{H}_{2}})\, /\, n_H $'
    plt.ylabel(ylab, fontsize=fontsize)
    plt.title('HI and Hmol fraction of hydrogen for different UVB models')
    plt.legend()
    
    plt.subplot(122)
    plt.plot(np.log10(nHs), np.log10(nHIHmol_over_nH(dct_nH_T, 5.,
                                                     UVB='HM01',
                                                     useLSR=False)),
             label=r'$z=5$', color='maroon', linestyle='solid')
    plt.plot(np.log10(nHs), np.log10(nHIHmol_over_nH(dct_nH_T, 4.,
                                                     UVB='HM01',
                                                     useLSR=False)),
             label=r'$z=4$', color='red', linestyle='dashed')
    plt.plot(np.log10(nHs), np.log10(nHIHmol_over_nH(dct_nH_T, 3.,
                                                     UVB='HM01',
                                                     useLSR=False)),
             label=r'$z=3$', color='brown', linestyle='dashed')  
    plt.plot(np.log10(nHs), np.log10(nHIHmol_over_nH(dct_nH_T, 2., 
                                                     UVB='HM01',
                                                     useLSR=False)),
             label=r'$z=2$', color='teal', linestyle='dashdot')
    plt.plot(np.log10(nHs), np.log10(nHIHmol_over_nH(dct_nH_T, 1.,
                                                     UVB='HM01',
                                                     useLSR=False)),
             label=r'$z=1$', color='darkblue', linestyle='dashdot')
    plt.plot(np.log10(nHs), np.log10(nHIHmol_over_nH(dct_nH_T, 0., 
                                                     UVB='HM01', 
                                                     useLSR=False)), 
             label=r'$z=0$', color='lightcoral', linestyle='dotted')
    plt.xlim((-5.1, 0.2))
    plt.ylim((-8.1, 0.25))
    plt.xlabel(r'$\log_{10} n_H \, [\mathrm{cm^{-3}}]$', fontsize=fontsize)
    ylab = r'$\log_{10} (n_{\mathrm{HI}} + n_{\mathrm{H}_{2}})\, /\, n_H $'
