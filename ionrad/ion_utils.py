...
class Linetable_PS20:
    '''
    class for storing data from the Ploeckinger & Schaye (2020) ion
    balance and line emission tables. 
    
    Methods include functions for interpolating the ion balance and
    emissvitiy tables in logT, logZ, and lognH, as well as the dust 
    depletion and assumed abundance of the given element for a total
    metallicity value. These table interpolations omit the 
    log Z / solarZ = -50.0 values in the tables, because these somewhat
    arbitrary zero values are unsuitable for interpolation.
    
    This does mean that line emission (proportional to the element
    fraction) should be rescaled to the right metallicity (or better: 
    specific element abundance) after the interpolation step.
    
    A single instance is for one emission or absorption line. This
    does mean the table bins and dust table may be stored twice if
    multiple objects are in use. The instance is also only valid for
    one redshift.
    '''
    
    def parse_ionname(self):
        '''
        retrieve the element name and ionization stage by interpreting
        the ion (or emission line) string. Assumes that emission lines
        match the format of the IdentifierLines dataset in the emission
        line tables (and are not blends), and that absorption lines are
        speciefied as e.g. 'o7', 'fe17', or 'Si 4', possibly followed 
        by something other than digits
        ...
        '''
        # lines are formatted as in the IdentifierLines dataset
        if self.emission: 
            try:
                self.elementshort = self.ion[:2].strip()
                self.ionstage = int(self.ion[2:4])
                msg = ('Interpreting {line} as coming from the '
                       '{elt} {stage} ion')
    ...
    def getmetadata(self):
        ...
        the abundance of the selected element as a function of 
        log Z / solarZ (self.numberfraction_Z),
        ...
        with h5py.File(self.ionbalfile, 'r') as f:
            # get element abundances (n_i / n_H) 
            # for tabulated log Z / Z_solar
            ...
            self.eltind = np.where([self.elementshort == elt \
                                    for elt in elts])[0][0]
            # element number fraction n_i / n_H as a function of 
            # metallicity in the tables
    ...
    def __init__(self, ion, z, emission=False, vol=True,
        ...
        ----------
        ion: string
            ion or emission line to get tables for; emission line names
            should match an entry in the IdentifierLines dataset in 
            the emission line table. Ion names should follow the format
            '<element abbreviation><ionization stage>[other stuff]',
            e.g. 'o7' for the O^{6+} / O VII ion, 'Fe17-other', etc.
            other options are 'hmolssh' and 'h1ssh'. Note that 'h2' is
            ionized hydrogen (H^+), not molecular hydrogen (H_2)
        z: float
            redshift. Must be within the tabulated range.
        emission: bool
            get emission tables (True) or absorption tables (False)
            if emission is True, the absorption table methods for the
            ion producing the line are also available. Using
            emission=True for an invalid emission line name will
            produce an error though.
        vol: bool
            Use quantities for the last Cloudy zone (True) or
            column-averaged quantities (False). The column option is
            only available for the hydrogen species.
        ionbalfile: string
            the file (including directory path) containing the ion
            balance data. This is also needed for emission
            calculations, since some of the metadata is not contained
            in the emission file.
        emtabfile: string
            the file (including directory path) containing the
            emissivity data. Only used when calculating emission data.
            It is assumed table options, like inclusion (or not) of
            shielding, cosmic rays, dust, and stellar radiation, are
    ...
    def __repr__(self):
        _str = ('{obj} instance: '
                'interpolate Ploeckinger & Schaye (2020) tables\n')
        _str += ('ion:\t {ion}, '
                 'interpreted as (coming from) the {elt} {stage} ion\n')
        _str += 'z:\t {z}\n'
        _str += 'vol:\t {vol}\n'
    ...
    def find_ionbal(self, dct_T_Z_nH, log=False):
        '''
        retrieve the interpolated ion balance values for the input 
        particle density, temperature, and metallicity
        
        The lowest metallicity bin is ignored since all ion fractions
        are tabulated as zero there.
    ...
    def find_logemission(self, dct_T_Z_nH):
        '''
        retrieve the interpolated emission values for the input 
        particle density, temperature, and metallicity
    ...
    def find_depletion(self, dct_T_Z_nH):
        '''
        retrieve the interpolated dust depletion values for the input
        particle density, temperature, and metallicity
    ...
    def find_assumedabundance(self, dct_Z, log=False):
        '''
        retrieve the interpolated assumed parent element abundance
        values for the input particle metallicity
        ...
        -------
        float array
            element abundance n_i / n_H for the parent element of the
            line or ion assumed at the input metallicity
    ...
    def findiontable(self):
        ...
        with h5py.File(self.ionbalfile, "r") as tablefile:
            ...
            tableg = tablefile[tablepath] #  z, T, Z, nH, ion
             # 0: Redshift, 1: Temperature, 2: Metallicity, 
             # 3: Density, 4: Ion
            if zi_lo == zi_hi:
                self.iontable_T_Z_nH =\
    ...
    def findemtable(self):
        ...
        with h5py.File(self.emtabfile, 'r') as f:
            ...
            self.lognHcm3 = f['TableBins/DensityBins'][:]
             # -50. = primordial
            self.logZsol = f['TableBins/MetallicityBins'][1:]
    ...
    def interpolate_3Dtable(self, dct_logT_logZ_lognH, table):
        '''
        retrieve the table values for the input particle density,
        ...
        ValueError
            ...
            or the input table shape doesn't match 
            logTK, logZsol, lognHcm3.
        ...
        if len(logT) != NumPart or len(logZ) != NumPart:
            msg = 'lognH, logZ, and logT  should have the same length'
            raise ValueError(msg)
    
        # need to compile with some extra options to get this to work:
        # make -f make_emission_only
        msg = ('------------------- C interpolation function output'
               ' --------------------------\n')
        print(msg)
        ...
        msg = ('-------------- C interpolation function output finished'
               ' ----------------------\n')
        print(msg)
    
        if res != 0:
            msg = f'Something has gone wrong in the C function: output {res}.'
